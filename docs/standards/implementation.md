...
| Paper v2 quality hierarchy | Use provider-native daily observations for formation, observed-only fixed 15-minute tokens for JEPA/forecasting, and exact consumed-window minutes for TCA | Data validity must match the scientific task's resolution; absence of a provider minute aggregate is not proof of zero activity |
