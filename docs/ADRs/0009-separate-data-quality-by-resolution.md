...
Every quality record stores `daily_valid`, `minute_exact_full_session`, `token_valid_full_session`, `tca_window_exact`, `early_close`, `provider_gap_count`, `observed_minute_count`, `valid_token_count`, `invalid_token_reason`, `token_observed_bar_counts`, and `token_provider_gap_counts` separately. SPY uses the same task-resolution contracts. Target validation, stock and SPY sequence construction, and causal seasonal aggregation all select this protocol from the authoritative sequence configuration. Sequence symbols are session-active observations verified against sourced stable-identity intervals, not copied from the formation snapshot.
